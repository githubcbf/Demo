// form ================================================================
odoo.define('sco_base.SCOForm', function (require) {
    "use strict";
    ...
    FormController.include({
        ...
        _onButtonClicked: function (event) {
            // console.log("TAG ------- _onButtonClicked event ", event);
            // console.log("TAG ------- _onButtonClicked this ", this);
            let isCusHandle = false;
            let self = this;
            // dms_params="{
            //                                 'btn_id':'btn_dms_aftersale_add_maintenance_materials_wizard'
            //                             }"
            try {
                let dmsParams = event.data.attrs.dms_params;
                let reg = new RegExp("'", "g");
                // console.log("TAG ------- dmsParams ", dmsParams);
                let parse = JSON.parse(dmsParams.replace(reg, "\""));
                // console.log("TAG ------- parse ", parse);
                if (parse.btn_id === "btn_dms_aftersale_add_maintenance_materials_wizard") {
        ...
        _update: function (state) {
            ...
            try {
                // 获取到自定义数据
                var modifiers = this.renderer.arch.attrs.modifiers;
                var evalModifiers = state.evalModifiers(modifiers);
                if (evalModifiers) {
                    /*
                    sco_form_edit_create_hide: 是否隐藏顶部 新增 编辑  按钮
                    sco_form_edit_hide: 是否隐藏顶部  编辑  按钮
                    sco_form_create_hide: 是否隐藏顶部 新增   按钮
                    例:
                         <form string="Deploy Project Task" class="o_form_project_tasks"
                              delete="false" duplicate="false"
                              attrs="{'sco_form_edit_create_hide' : [('temp_stage_tag', '==', 'sco_stage_30')]}"
                            >
                 */
                    //隐藏 编辑和创建按钮
                    if (evalModifiers.hasOwnProperty("sco_form_edit_create_hide")) {
                        let scoFormEditCreateHide = evalModifiers.sco_form_edit_create_hide;
                        // console.log('FormController _update scoFormEditCreateHide', scoFormEditCreateHide);
                        if (scoFormEditCreateHide) {
                            //主action toolbar
                    ...
                    //隐藏编辑按钮
                    if (evalModifiers.hasOwnProperty("sco_form_edit_hide")) {
                        let scoFormEditHide = evalModifiers.sco_form_edit_hide;
                        // console.log('FormController _update scoFormEditHide', scoFormEditHide);
                        if (scoFormEditHide) {
                            // 顶部编辑按钮
                    ...
                    //隐藏创建按钮
                    if (evalModifiers.hasOwnProperty("sco_form_create_hide")) {
                        let scoFormCreateHide = evalModifiers.sco_form_create_hide;
                        // console.log('FormController _update scoFormCreateHide', scoFormCreateHide);
                        if (scoFormCreateHide) {
                            //顶部创建按钮
                ...
                console.log("ERROR FormController _update hide Btns e", e);
            }
         ...
         * 错误弹框
         */
        cusDisplayError: function (title, message) {
            this.cusDialogOpen({
                title: title,
    ...
    FormRenderer.include({
        ...
        _activateNextFieldWidget: function (record, currentIndex) {
            ...
            解决 form 页面回弹问题
             */
            // console.log("SCO === FormRenderer _activateNextFieldWidget === record: ", record);
            // console.log("SCO === FormRenderer _activateNextFieldWidget === currentIndex: ", currentIndex);
            if ((currentIndex + 1) >= (this.allFieldWidgets[record.id] || []).length) {
                ...
                // console.log("SCO === FormRenderer _activateNextFieldWidget === activatedIndex: ", activatedIndex);
            ...
            // console.log("SCO === FormRenderer _activateNextFieldWidget === this.lastActivatedFieldIndex: ", this.lastActivatedFieldIndex);
...
odoo.define('sco_base.SCOFormTreeView', function (require) {
    ...
    var SCOFormTreeView = FormView.extend({
        jsLibs: [],
